...
DEFAULT_TRAINING_FEATURE_COLUMNS_PATH = Path(
...
def load_default_feature_columns() -> list[str]:
    return load_feature_columns(DEFAULT_FEATURE_COLUMNS_PATH)
...
class FlowFeatureContract:
    ...
    def from_feature_file(
        cls,
        path: Path = DEFAULT_FEATURE_COLUMNS_PATH,
        alias_map: Mapping[str, str] | None = None,
    ) -> "FlowFeatureContract":
...
def load_default_contract(
    ...
    return FlowFeatureContract.from_feature_file(
        path=DEFAULT_FEATURE_COLUMNS_PATH,
        alias_map=alias_map,
    )
