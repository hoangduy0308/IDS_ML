...
def test_evaluate_iot_diad_two_stage_gated_writes_report(tmp_path: Path) -> None:
    ...
    assert "zero_pass_through" in report["gated_evaluation"]["ood_attack_holdout"]["by_true_family"]["BruteForce"]
    assert "zero_pass_through" in report["gated_evaluation"]["ood_attack_holdout"]["by_true_family"]["Recon"]
